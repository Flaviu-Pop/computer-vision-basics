...
train_dataset_augmented = torchvision.datasets.ImageFolder(train_dataset_path, transform=transform_augmentation)
test_dataset_augmented = torchvision.datasets.ImageFolder(test_dataset_path, transform=transform_augmentation)

# We do the unions of the corresponding datasets, in fact we double the datasets
train_dataset = train_dataset.__add__(train_dataset_augmented)
# test_dataset = test_dataset.__add__(test_dataset_augmented)
...
class CnnBinaryClassification(nn.Module):
    def __init__(self):
        ...
        self.convolutional_layer_1 = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, bias=False,
                                            dtype=torch.float64)
        self.max_pooling_1 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.convolutional_layer_2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, bias=False,
                                            dtype=torch.float64)
        self.max_pooling_2 = nn.MaxPool2d(kernel_size=2, stride=2)

        self.flatten_layer = nn.Flatten()

        self.linear_layer_1 = nn.Linear(in_features=6272, out_features=128, dtype=torch.float64)
        self.linear_layer_2 = nn.Linear(in_features=128, out_features=16, dtype=torch.float64)
        self.linear_layer_3 = nn.Linear(in_features=16, out_features=1, dtype=torch.float64)
